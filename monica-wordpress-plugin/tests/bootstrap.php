...
function reset_mock_calls() {
    ...
    $mock_calls = [
        ...
        'Monica_API_get_return' => ['data' => [['body' => 'Mocked note']]],
...
function get_transient($transient) {
    global $mock_transient;
    return isset($mock_transient[$transient]) ? $mock_transient[$transient] : false;
}

function set_transient($transient, $value, $expiration = 0) {
    global $mock_transient;
    $mock_transient[$transient] = $value;
}

function delete_transient($transient) {
    global $mock_transient;
    if (isset($mock_transient[$transient])) {
        unset($mock_transient[$transient]);
        return true;
    }
    return false;
}
...
class Monica_API {
    public function get($endpoint, $args = []) { if (defined("BENCHMARK_SLOWDOWN")) usleep(500000);
        global $mock_calls;
