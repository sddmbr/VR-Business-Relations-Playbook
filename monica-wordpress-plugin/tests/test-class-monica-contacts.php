...
function test_save_contact_meta_data_update_existing_api_error() {
    $contacts = new Monica_Contacts();
    global $mock_calls;
    ...
    // Simulate API error (though the current code doesn't check is_wp_error on put,
    // we set it just in case, or to reflect potential future updates like the post one)
    $mock_calls['is_wp_error_return'] = true;
